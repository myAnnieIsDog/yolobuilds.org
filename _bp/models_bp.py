...
from _base.models import Record
...
class BP_Base(Record):
    number = models.CharField(max_length=55, unique=True)
    valuation = models.DecimalField(max_digits=15, decimal_places=2, default=2000)
    use_group = models.ForeignKey(
        UseGroup, on_delete=models.PROTECT, related_name="%(app_label)s_%(class)s_use"
    )
    type_of_construction = models.ForeignKey(
        TypeOfConstruction,
        on_delete=models.PROTECT,
        related_name="%(app_label)s_%(class)s_type_of_const",
    )
    ...
    CSLB_Forms = models.BooleanField("CSLB License Verification", default=False)
    employee_authorization = models.BooleanField(
        "CSLB Licensed Employer Authorization of Employee to Pull Permits",
        default=False,
    )
